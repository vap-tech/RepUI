...
import { initNavigationMenus, NavigationMenu } from './components/navigation-menu.js';
import { initPagination } from './components/pagination.js';
import { initCodeBlocks, highlightHtml, highlightCss, highlightDjango, highlightCode } from './components/code-block.js';

let currentApi = null;
function registerCollection(collection, name) {
  if (collection instanceof Map) collection.forEach(instance => register(instance.root, name, instance));
  else if (Array.isArray(collection)) collection.forEach(instance => register(instance.root, name, instance));
}
export function init(root = document) {
  ...
  const api = {
    dialogs: initDialogs(root), toasts: initToasts(root), listboxes: initListboxes(root),
    selects: initSelects(root), comboboxes: initComboboxes(root), commands: initCommands(root),
    dropdowns: initDropdowns(root), popovers: initPopovers(root), sheets: initSheets(root),
    alertDialogs: initAlertDialogs(root), tabs: initTabs(root), accordions: initAccordions(root),
    collapsibles: initCollapsibles(root), menubars: initMenubars(root), navigationMenus: initNavigationMenus(root),
  };
  Object.entries(api).forEach(([name, collection]) => registerCollection(collection, name.replace(/s$/, '')));
  initAlerts(root); initLoadingButtons(root); initTooltips(root); initValidation(root); initSearch(root); initPagination(root); initCodeBlocks(root);
  currentApi = api;
...
const RepUI = {
  version: '0.8.2.5', init, destroy, setTheme, getTheme, getModality,
  open: interaction.open, close: interaction.close, toggle: interaction.toggle, state: interaction.state,
  getInstance, getComponentName,
  portal: { mount: mountPortal, unmount: unmountPortal, isPortaled },
  Dialog, Listbox, Select, Combobox, CommandPalette, Tabs, Accordion, Collapsible,
  Menubar, NavigationMenu, ToastManager, DropdownMenu, Popover, Sheet, AlertDialog,
  debug: setDebug, inspect: inspectComponent, highlightHtml, highlightCss, highlightDjango, highlightCode,
  toast(options) { return (currentApi?.toasts || new ToastManager()).show(options); },
};
window.RepUI = RepUI;
